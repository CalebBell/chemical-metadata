...
mycache = FileCache('myapp', flag='cs', serialize=True, app_cache_dir='/home/caleb/Documents/University/CHE3123/chemical-metadata/pubchempy')
...
def request(identifier, namespace='cid', domain='compound', operation=None, output='JSON', searchtype=None, **kwargs):
    ...
    try:
        postdata2 = postdata.decode('utf-8')
    except:
        postdata2 = str(postdata)
    key = apiurl + postdata2
...
#            print('not in cache', apiurl, postdata, type(apiurl), type(postdata))
...
#            print('response read')
        ...
        mycache[key] = text
        text = mycache[key]
#        print('text', text, 'looked up from cache')
        
        
        if text is None:
            raise Exception({'code': 404, 'msg': ''})
#        print('no exception')
...
class Compound(object):
    ...
    def from_cid(cls, cid, **kwargs):
        ...
        """
        f = request(cid, **kwargs).read()
        try:
            record = json.loads(f.decode())['PC_Compounds'][0]
        except:
            record = json.loads(f)['PC_Compounds'][0]
        return cls(record)
